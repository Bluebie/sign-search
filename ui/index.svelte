<svelte:head>
  ...
  <meta property="og:description" content="Find Sign is an Auslan Search Engine. It helps find Auslan resources from around the internet in one place. Deaf community can send websites and videos to include and help make Auslan vocabulary easier to find.">
...
<style>
  ...
  :root {
    ...
    --alert-bg-hue: calc(var(--hue) - 40deg);
    --alert-bg: hsl(var(--alert-bg-hue), var(--module-bg-sat), var(--module-bg-lum));
    --submodule-bg-sat: 21%;
    --submodule-bg-lum: 24%;
    --submodule-bg: hsl(var(--hue), var(--submodule-bg-sat), var(--submodule-bg-lum));
    --outer-shadow-alpha: 20%;
    --outer-highlight-alpha: 40%;

    --fade-duration: 500ms;
    --fade-time-offset: 50ms;
  ...
  @media screen and (prefers-reduced-motion: reduce) {
    /* disable motion on devices which request that, even though it makes search results appear chunky and weird */
    :root {
      --fade-duration: 0ms;
      --fade-time-offset: 0ms;
    }
  }

  @media (prefers-color-scheme: light) {
    /*
    opt-in light-mode theme for accessibility sake
    changes some page-wide colors, and applies some filters to fix some
    of the symbols to look right
    */
    :root {
      --base-bg-sat: 43%;
      --base-bg-lum: 90%;
      --base-fg-sat: 20%;
      --base-fg-lum: 22%;
      /* --filter-fg: saturate(150%) brightness(25%); */
      --module-bg-sat: 35%;
      --module-bg-lum: 83%;
      --submodule-bg-sat: 33%;
      --submodule-bg-lum: 75%;
      --outer-shadow-alpha: 40%;
      --outer-highlight-alpha: 100%;
    }
  }
