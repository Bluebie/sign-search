...
class SearchLibraryWriter {
  ...
  constructor(basePath, settings = {}) {
    ...
    this.settings = {
      ...
      gzip: settings.gzip || false // TODO: Implement static gzipping in version 3 for def files
    }
    if (this.settings.buildID === undefined) this.settings.buildID = Math.floor(this.settings.buildTimestamp).toString(36)
    this.keepRecentBuilds = settings.keepRecentBuilds || 3 // how many recent builds to leave defs for
  ...
  async append({words, tags: unfilteredTags, def, videoPaths}) {
    ...
    fs.writeJSON(definitionFilePath, existingDefinitions)
  }
