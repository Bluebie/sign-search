  ...
  async getCachePath(inputVideo, options = {}) {
    ...
    if (!(await fs.pathExists(videoPath)) || options.force) {
      ...
      try {
        ...
        await this.transcode(rawVideoPath, tempVideoPath, start, end)
...
class SearchLibraryWriter {
  ...
  async open() {
    try { await fs.remove(this.paths.defs) } catch {}
    this.symbols = {}
    this.symbolsIndexData = []
    this.symbolPackets = []
    this.entries = {}
    this.skipBuild = await fs.pathExists(this.paths.defs)
  ...
  async append({words, tags: unfilteredTags, def, videoPaths}) {
    ...
    // figure out path for this definition data
    let defPath = `${this.paths.defs}/${prefix}/${identifier}`
    await fs.ensureDir(defPath) // ensure directories exist
    ...
    let jsonGzip = await util.promisify(zlib.gzip)(json, { level: 9 })
    let writeTasks = [fs.writeFile(`${defPath}/definition-${nextID}.json`, json)]
    // if compressed file is actually smaller, write it out
    if (jsonGzip.byteLength < json.byteLength) writeTasks.push(fs.writeFile(`${defPath}/definition-${nextID}.json.gz`, jsonGzip))
  ...
  async finish() {
    ...
    // erase old defs builds
    for (let dirName of (await fs.readdir(this.paths.defsBase))) {
      if (dirName != this.settings.buildID) await fs.remove(`${this.paths.defsBase}/${dirName}`)
    }
