...
class VideoCacheDefaultChecker {
  ...
  // simple implementation just sha256's the video's data from the local filesystem
    ...
    return new Promise((resolve, reject) => {
      ...
      hbjs.spawn(options).on('error', err => {
        ...
        if (Date.now() > lastProgress + progressFrequency && lastPercent != progress.percentComplete) {
          this.log(`Encode Percent complete: ${progress.percentComplete}, ETA: ${progress.eta}`)
...
class SearchLibraryWriter {
  ...
  constructor(basePath, settings = {}) {
    ...
    this.encoders = settings.encoders || [new HandbrakeEncoder({ log: this.log })]
    for (let encoder of this.encoders) {
    ...
    this.videos = new VideoCache({path: this.paths.videos, hash: (data)=> this.hash(data), encoders: this.encoders, log: this.log})
  }
  ...
  async open() {
    ...
    await fs.ensureDir(this.paths.defsBase)
    await fs.ensureDir(this.paths.videos)
    ...
    if ((await fs.pathExists(this.paths.defs)) && (await fs.pathExists(this.paths.index))) {
      ...
      await reader.open(this.paths.base)
      if (reader.settings.buildID == this.settings.buildID) this.skipBuild = true
    }
    if (this.skipBuild) this.log("This Search Library version already exists, skipping rebuild, no files will be written")
    else await fs.ensureDir(this.paths.defs) // make sure the defs directory exists
    return this
  }
  ...
  async append({words, tags: unfilteredTags, def, videoPaths: videoProviders}) {
    ...
    let media = []
    for (let videoProvider of videoProviders) {
      let cachePaths = await this.videos.getCachePaths(videoProvider, {force: this.redownloadChance > Math.random()})
      let variationData = {}
