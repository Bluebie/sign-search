...
class VideoCache {
  ...
  async getCachePath(inputVideo) {
    ...
    if (!(await exists(`${this.path}`))) await fs.mkdir(`${this.path}`)
    if (!(await exists(`${this.path}/${prefix}`))) await fs.mkdir(`${this.path}/${prefix}`)
  ...
  async eraseUnused() {
    for (let prefix of await fs.readdir(this.path)) {
      ...
      if ((await fs.readdir(`${this.path}/${prefix}`)).length == 0) await rmdir(`${this.path}/${prefix}`)
    }
  }
}
...
class SearchLibraryWriter {
  ...
  async open() {
    try { await rmdir(this.paths.defsTmp) } catch {}
    return this
  }
  ...
  async append({words, tags: unfilteredTags, def, videoPaths}) {
    ...
    if (!(await exists(`${this.paths.defsTmp}`))) await fs.mkdir(`${this.paths.defsTmp}`)
    if (!(await exists(`${this.paths.defsTmp}/${prefix}`))) await fs.mkdir(`${this.paths.defsTmp}/${prefix}`)
    if (!(await exists(`${this.paths.defsTmp}/${prefix}/${identifier}`))) await fs.mkdir(`${this.paths.defsTmp}/${prefix}/${identifier}`)
  ...
  async finish() {
    ...
    if (await exists(this.paths.defs)) await fs.rename(this.paths.defs, `${this.paths.defs}-trash`)
    await fs.rename(this.paths.defsTmp, this.paths.defs)

    // erase old trash defs directory
    if (await exists(`${this.paths.defs}-trash`)) await rmdir(`${this.paths.defs}-trash`)
