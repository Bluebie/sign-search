...
class SearchLibraryWriter {
  ...
  async append({words, tags: unfilteredTags, def, videoPaths}) {
    ...
    let tagList = tags.map(string => this.symbol(string)).join(",")
    ...
    let identifier = this.hash(Buffer.concat(symbolList.map(symID => this.symbolsIndexData[symID]))).toString('hex').toLowerCase()
