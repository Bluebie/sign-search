...
const BuildPacket = {
  ...
  defineSearchResult: (symbols, definitions)=> {
    ...
    buffer.writeUInt8(9, 0) // packet type
    buffer.writeUInt8(symbols.length, 1)
    buffer.writeUInt8(definitions, 2)
    symbols.forEach((symbol, index)=>
      buffer.writeUInt16BE(symbol, 3 + (index * symbolSize))
    )
    return buffer
  }
}
...
class SearchLibraryWriter {
  ...
  async append({words, tags: unfilteredTags, def, videoPaths}) {
    ...
    for (let word of words.flat()) {
      ...
      if (!this.symbols[cleanedWord]) { // if symbols aren't already defined, define them
        if (vector) {
          let vectorData = this.symbolsIndexData[cleanedWord] = this.compressVector(vector)
          this.symbolPackets.push(BuildPacket.establishVectorSymbol(this.symbolsIssued, vectorData))
          console.log(`VECTOR Symbol: ${this.symbolsIssued}: "${cleanedWord}"`)
        } else {
          this.symbolsIndexData[cleanedWord] = Buffer.from(cleanedWord)
          this.symbolPackets.push(BuildPacket.establishStringSymbol(this.symbolsIssued, cleanedWord))
          console.log(`STRING Symbol: ${this.symbolsIssued}: "${cleanedWord}"`)
        }
        this.symbols[cleanedWord] = this.symbolsIssued
        
        this.symbolsIssued += 1
      }
      let symbolID = this.symbols[cleanedWord]
      symbolList.push(symbolID)
      symbolDataList.push(this.symbolsIndexData[cleanedWord])
    }

    let identifier = this.hash(Buffer.concat(symbolDataList)).toString('hex').toLowerCase()
    let prefix = identifier.slice(0, 2)

    if (!this.entries[tagList][identifier]) this.entries[tagList][identifier] = { symbols: symbolList, definitions: 0 }
  ...
  async finish() {
    ...
    for (let tagList of Object.keys(this.entries)) {
      if (tagList.toString() != "") {
        ...
        // reset the readers lag list and add every tag used for this section
