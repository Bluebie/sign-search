...
class SearchLibraryWriter {
  ...
  async append({words, tags: unfilteredTags, def, videoPaths}) {
    ...
    await Promise.all([
      fs.writeFile(`${defPath}/definition-${nextID}.json`, json),
      fs.writeFile(`${defPath}/definition-${nextID}.json.gz`, jsonGzip)
    ])
  }
  ...
  async finish() {
    ...
    // only rewrite if contents changed, to preserve downstream caches
