...
class SearchResultDefinition extends SearchDefinition {
  constructor({ library, tags, terms, termDiversity, hash, definitionIndex } = {}) {
    super()
    this.library = library
    this.tags = tags || []
    this.terms = terms || []
    this.termDiversity = termDiversity || 0
    this._hash = hash
    this._definitionIndex = definitionIndex || 0
    this._fetched = false
  }
  ...
  async fetch() {
    if (!this._fetched) {
      ...
      storage.media    = chunk.media.map(formats => 
        Object.keys(formats).map(extension => Object.create(this.library.settings.mediaSets.find(media => media.extension == extension), { url: { value: `${this.library.baseURL}/${formats[extension]}` }}) )
      )
