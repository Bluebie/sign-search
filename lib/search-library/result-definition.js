...
class SearchResultDefinition extends SearchDefinition {
  ...
  async fetch() {
    ...
    this.media    = chunk.media.map(formats => 
      Object.keys(formats).map(extension => Object.assign(Object.create(this.library.settings.mediaSets.find(media => media.extension == extension) || {}), { url: `${this.library.path}/media/${formats[extension]}` }) )
    )
