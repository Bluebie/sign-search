...
class SearchResultDefinition extends SearchDefinition {
  ...
  async fetch () {
    ...
    // )
    this.media = []
    for (const chunkFormats of chunk.media) {
      const mediaItem = []
      for (const configuredFormat of this.library.settings.mediaSets) {
        if (chunkFormats[configuredFormat.extension]) {
          mediaItem.push({
            ...configuredFormat,
            url: `${this.library.webURL}/media/${chunkFormats[configuredFormat.extension]}`
          })
        }
      }
