...
class FFMPEGEncoder {
  ...
  async encode (inputPath, outputPath, start, end) {
    ...
    const ffmpegExitCode = new Promise((resolve, reject) => {
      ffmpeg.on('exit', (code) => {
        if (code && code !== 0) reject(new Error(`ffmpeg child process exited with error ${code}`))
        else resolve()
      })
    })
    const linesOutput = byline.createStream(ffmpeg.stderr)
    for await (const line of linesOutput) {
      this.log(line)
    }
