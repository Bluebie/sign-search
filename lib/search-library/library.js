...
class SearchLibrary {
  ...
  constructor(config = {}) {
    ...
    this.log = config.log || ((...args) => console.log(...args))

    // configure log functions on mediaFormats
    config.mediaFormats = (config.mediaFormats || []).map(originalFormat => {
      let modifiedFormat = Object.create(originalFormat)
      modifiedFormat.log = (...args) => this.log(...args)
    ...
    // internal function to write compressed with gzip and brotli if available
    this.writeCompressed = (filename, buffer) => {
      let tasks = []
      config.fs.writeFile(filename, buffer)
      if (config.gzip) {
        let gzipWriter = async () => config.fs.writeFile(`${filename}.gz`, await config.gzip(buffer)) 
        tasks.push(gzipWriter())
      }
      if (config.brotli) {
        let brotliWriter = async () => config.fs.writeFile(`${filename}.br`, await config.brotli(buffer)) 
  ...
  async cleanup() {
    ...
    for (let buildID of builds) {
      this.log(`Checking build dir "${buildID}" against "${this.settings.buildID}"`)
      if (buildID !== this.settings.buildID) {
        this.log(`Nuking ${buildID}`)
