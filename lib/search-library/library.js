...
class SearchLibrary {
  ...
  constructor(config = {}) {
    ...
    assert(this.path, "path option is not optional") // verify a path was supplied
    this.log = config.log || (text => console.log(text))
  }
  ...
  async open() {
    ...
    assert.equal(data.settings.version, 4)
  ...
  async addDefinition(definition) {
    ...
    for (let mediaItem of data.media) {
      ...
      for (let encoder of capableEncoders) {
        versions[encoder.getMediaInfo().extension] = await this.config.mediaCache.cache({ media: mediaItem, format: encoder })
      }
  ...
  async save() {
    ...
    let index = {} // object keyed by tag lists in the form "symbol\nsymbol\nsymbol" where symbols are sorted numbers from symbol()
    {
      ...
      for (let batch of Util.chunkIterable(this.index, this.config.definitionsPerShard)) {
        ...
        batch.forEach(definition => {
          ...
          // build tag list
          let tagList = definition.tags.map(symbol).sort().join('\n')
          ...
          let symbolList = definition.terms.map(symbol).sort().join('\n')
        ...
        await this.config.fs.writeFile(`${this.path}/definitions/${this.settings.buildID}/${shardID}.cbor`, shardBuffer)
        if (this.config.gzip) await this.config.fs.writeFile(`${this.path}/definitions/${this.settings.buildID}/${shardID}.cbor.gz`, await this.config.gzip(shardBuffer))
        shardID += 1
      }
    }
    ...
    await this.config.fs.writeFile(`${this.path}/index.cbor`, indexBuffer)
    if (this.config.gzip) await this.config.fs.writeFile(`${this.path}/index.cbor.gz`, await this.config.gzip(indexBuffer))
  ...
  async cleanup() {
    ...
    builds.forEach(buildID => {
      if (buildID !== this.settings.buildID) this.config.fs.remove(`${this.path}/definitions/${buildID}`)
    })
    // clean up old media in the cache that is no longer referenced
    await this.mediaCache.cleanup()
  }
