...
class MediaCache {
  ...
  async cache({ media, format }) {
    ...
    let encodePath     = `${this.path}/media/${shardID}/partial-${outputFilename}`
    let videoPath      = `${this.path}/media/${shardID}/${outputFilename}`
    let relativePath   = `${shardID}/${outputFilename}`
  ...
  async cleanup() {
    for (let prefix of await fs.readdir(`${this.path}/media`)) {
      ...
      for (let videoFilename of await fs.readdir(`${this.path}/media/${prefix}`)) {
