...
class SearchLibraryReader {
      ...
      "define search result": async (data)=> {
        ...
        for (let recordID = 0; recordID < data.definitions; recordID++) {
          this.searchIndex.push({
            hash, index, tags, terms, recordID, termDiversity,
  ...
  //   rankMode: (string) one of [mixmean, mean, min, max, add], defines how the distances are calculated when search includes multiple queries
  async lookup(search, options = {}) {
    if (this.settings.version < 1 || this.settings.version > 3) throw new Error("Unsupported version")
    ...
    // if we have a search, calculate distances or approximate with plaintext searches
    let results = {}
    ...
    for (let entity of this.searchIndex) {
      ...
      let searchDistances = search.map(searchTerm => {
        return Math.min(...entity.terms.map(entityTerm => {
          ...
          if (entityTerm.constructor == Array || entityTerm.constructor == Uint8Array) {
            foundResults = true
            return VU.distanceSquared(entityTerm, searchTerm)
          } else if (entityTerm.constructor == String) {
            if (searchTerm == entityTerm) {
              foundResults = true
              return 0
            } else if (searchTerm.toLowerCase() == entityTerm.toLowerCase()) {
              return 0.25
            }
          } else {
      ...
      if (options.rankMode == 'mixmean') {
        result.distance = searchDistances.reduce((x,y) => x + y) * ((entity.terms.length + searchDistances.length) / 2)
      } else if (options.rankMode == 'mean') {
        result.distance = searchDistances.reduce((x,y) => x + y) / entity.terms.length
      } else if (options.rankMode == 'min') {
        result.distance = Math.min(...searchDistances)
      } else if (options.rankMode == 'max') {
        result.distance = Math.max(...searchDistances)
      } else if (options.rankMode == 'add') {
        result.distance = searchDistances.reduce((x,y) => x + y)
      } else {
        throw new Error('Unknown rankMode, must be "mixmean", "mean", "min", "max", or "add"')
      }
    }
  ...
  async fetchDef(result) {
    if (this.settings.version == 2) { // LEGACY: Deprecated
      let data = await this._fetch(`${this.getBasePath(result)}.json`)
      ...
      json.media = []
      for (let variation = 0; variation < json.variations; variation++) {
        json.media.push([
          { extension: '.mp4', url: `${this.getBasePath(result)}-video-${variation}.mp4`, type: 'video/mp4' }
        ])
      }

      json.result = result
      return json
    } else if (this.settings.version == 3) {
      let prefixBytesAB = result.entity.hash.slice(0, Math.ceil(this.settings.shardBits / 8))
      let prefixBytes = new Uint8Array(prefixBytesAB)
      let binaryString = [...prefixBytes].map(byte =>
        ('0'.repeat(8) + byte.toString(2)).slice(-8)
      ).join('').slice(0, this.settings.shardBits)
      let shardID = parseInt(binaryString, 2)
      
      let data = await this._fetch(`${this.baseURL}/defs/${this.settings.buildID}/${shardID}.json`)
      let shardJSON = JSON.parse((new TextDecoder()).decode(data))
      let json = shardJSON[result.entity.index][result.entity.recordID]
      json.result = result

      // rebuild the media object with settings data included for convenience
      let mediaList = json.media
      json.media = mediaList.map(mediaItem => Object.keys(mediaItem).map(extension =>
        Object.create(this.settings.mediaSets.find(x => x.extension == extension), { url: { value: `${this.baseURL}/${encodeURI(mediaItem[extension])}` }})
      ))
      json.variations = json.media.length

      return json
    } else {
      throw new Error("Search Index is incompatible version")
    }
  }

  // get a video link for a definition & varient from this dataset
  videoLink(result, variant = 0, suffix = ".mp4") {
    if (this.settings.version == 2) {
      return `${this.getBasePath(result)}-video-${variant}${suffix}`
    } else {
      throw new Error(".videoLink is only supported for v2 Search Index datasets")
    }
  }
