...
let PacketDecoder = [
  ...
  // uint8  packet type
  // uint8  tag byte length
  // string tag name
  (dataview, settings)=> ({
    type: 'add tag',
    length: 2 + dataview.getUint8(1),
    data: {
      tag: PacketDecoder.stringFromPosition(dataview, 2, dataview.getUint8(1))
    }
  }),

  // DEPRECATED type 4: set unknown words block
  // uint8  packet type
  // uint16 string length
  // string list of null seperated unknown words (words not in the vector database)
  (dataview, settings)=> ({
    type: 'set unknown words',
    length: 3 + dataview.getUint16(1),
    data: {
      unknownWords: PacketDecoder.stringFromPosition(dataview, 3, dataview.getUint16(1)).split("\0"),
      indexBytes: PacketDecoder.sliceSection(dataview, 3, dataview.getUint16(1))
    }
  }),

  // DEPRECATED type 5: define vector block
  // uint8  packet type
  // uint16 definition count for this search result
  // blob   vector data
  (dataview, settings)=> {
    return {
      type: 'define vector',
  ...
  // DEPRECATED type 6: null vector block
  // uint8  packet type
  // uint16 definitions stored to this search result
  (dataview, settings)=> ({
    type: 'null vector',
    length: 3,
    data: {
  ...
  // type 7: establish word vector (16bit symbols)
  // uint8  packet type
  // uint16 symbol index
  // blob   vector data
  (dataview, settings)=> ({
  ...
  // type 8: establish unknown word (16bit symbols)
  // uint8  packet type
  // uint16 symbol index
  ...
  // type 9: define search result (16bit symbols)
  // uint8  packet type
  // uint8  number of symbols
  // uint8  number of definitions stored to this search result
  // uint16[] symbol id's
  (dataview, settings)=> {
    let symbolCount = dataview.getUint8(1)
    let symbolSize = 2
    let symbols = []

    for (let i = 0; i < symbolCount; i++) {
      symbols.push(dataview.getUint16(3 + (i * symbolSize)))
    }

    return {
      type: 'define search result',
      length: 3 + (symbolCount * symbolSize),
      data: {
...
class SearchLibraryReader {
  ...
  async open(url) {
    ...
    let tags = []
    let unknownWords = [] // DEPRECATED
    let nullVectorIndexBytes = null // DEPRECATED
    let symbols = []
    ...
    let packetHandlers = {
      ...
      "set unknown words": async (data)=> { // DEPRECATED
        unknownWords = data.unknownWords
        nullVectorIndexBytes = data.indexBytes
      },
      "define vector": async (data)=> { // DEPRECATED
        ...
        for (let recordId = 0; recordId < data.definitions; recordId++) {
          this.searchIndex.push({
            ...
            tags: tags,
            terms: [data.vector, ...unknownWords]
          })
        }
        unknownWords = []
        nullVectorIndexBytes = null
      },
      "null vector": async (data)=> { // DEPRECATED
  ...
  async lookup(search, sort = false) {
    ...
    for (let searchTerm of search) {
      for (let entity of this.searchIndex) {
        for (let entityTerm of entity.terms) {
          if (typeof(entityTerm) != typeof(searchTerm)) continue
          let distance = 100
          if (entityTerm.constructor == Array) {
            distance = this._distSquared(entityTerm, searchTerm)
          } else if (entityTerm.constructor == String) {
            if (searchTerm == entityTerm) distance = 0
            else if (searchTerm.toLowerCase() == entityTerm.toLowerCase()) distance = 0.5
          } else {
            throw new Error('entity term type unknown')
          }
          if (!results[entity.url]) results[entity.url] = { id: entity.url, entity, distance, library: this }
          else if (results[entity.url].distance > distance) results[entity.url].distance = distance 
        }
      }
    }
