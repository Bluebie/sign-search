...
class SearchLibraryReader {
  ...
  constructor() {
    ...
    this.settings = {}
    this.mtime = null
    this.mtimeThreshold = 5000 // if data is more than 5 seconds newer than index, reload front end
  ...
  async fetchDef(result) {
    ...
    if (data.mtime > this.mtime + this.mtimeThreshold) throw new Error(`Dataset Updated: definition is ${(data.mtime - this.mtime) / 1000} seconds newer than loaded index`)
  ...
  async _fetchNodeJS(filename) {
    ...
    buffer.mtime = (await fs.stat(filename)).mtimeMs
    return buffer
  }
