    ...
    if (rawTerm.match(/^-#([a-zA-Z0-9-_]+)$/)) {
      ...
      try {
        ...
        if (err.toString().match(/Dataset Updated/)) {
          console.error(err)
          console.error("Vector library changed server side, requesting browser reload app")
          location.reload()
        } else {
          console.error(err)
          setView('notice', `Computer Problem with search: Vector lookup: ${err}`, 'data-error')
        }
      }
    }
  }
