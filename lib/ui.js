...
window.UIParsedSuccessfully = true
...
function onVisibilityChange() {
  if (document.hidden) $('video').each(el => el.pause())
  else $('video').each(el => el.play())
}
...
async function runQuery(search, start = 0) {
    ...
    if (rawTerm.match(/^-#([a-zA-Z0-9-_]+)$/)) {
      ...
      if (vector) {
        ...
        if (!rawTerm.match(/^-/)) {
          ...
          searchTerms.push(negativeTerm)
        }
        //plaintextTerms.push(cleanedTerm.toLowerCase())
      }
    }
  }
  ...
  for (let searchLibrary of signLibraries) {
    for (let result of (await searchLibrary.lookup(searchTerms, false))) {
...
function ariaNotice(message, mode = 'polite') {
  $("#aria-notice").remove()
  $("#results").before($('<div>').text(message).attr('aria-live', mode).css({"position":"absolute", "top": "-1000px"}))
}
...
function renderNotice(text, signtip) {
  ...
  document.title = originalTitle
  $("#results").empty().append($('<div>').text(text).attr('data-signtip', signtip.toString()).attr('aria-live', 'polite'))
}
...
async function renderResults() {
  ...
  for (let result of displayResults) {
    // is this the first result of the page?
    let isFirstResultOfPage = result == displayResults[0]
    // is this the first result of the first page?
    let isFirstResultOverall = result == searchResults[0]

    let template = $('<a class="result" referrerpolicy="origin"></a>')
    let videoWidget = $('<div class="video_player" data-current="0" data-variations="1" aria-hidden="true">')
    ...
    let rel = ['external']
    // browser should think about fetching the first result overallbecause user's most likely to choose that one
    if (isFirstResultOverall) rel.push('prefetch')
    template.attr('rel', rel.join(' '))

    resultsBox.append(template)

    loadingList.push(new Promise((resolve, reject)=> {
      result.library.fetchDef(result)
      .catch((err)=> {
        if (err.toString().match(/Dataset Updated/)) {
          console.log("Dataset changed after page load, requesting app reload")
          location.reload()
        } else {
          console.error(err)
          reject(err)
      ...
      .then(({variations, outOfDate, data: def})=> {
        ...
        template.find(".video_player").attr('data-variations', variations)
  
        // detected server side data probably was rebuilt while this page is open, should reload server side data
        if (outOfDate) {
          location.reload() // TODO: a smarter implementation that doesn't strobe the page with a full reload
        }
        ...
        let setVariant = (adjust)=> {
          ...
          let update = Math.min(Math.max(current + Math.round(adjust), 0), variations - 1) // constrain to sane values
          ...
          template.find(".video_player button.next_vid").css('display', update < (variations - 1) ? 'block' : 'none')
        }
        ...
        // if first result of page, focus element so user jumps to it, if using screenreader
        // TODO: figure out why this isn't working in iOS Safari VO - are links really not focusable??
        if (isFirstResultOfPage) template.focus()
  ...
  let maxPages = 8
  let pages = Math.min(maxPages, Math.ceil(searchResults.length / resultsShown))
  for (let pageID = 1; pageID <= pages; pageID++) {
    ...
    if (pageOffset / resultsShown == pageID - 1) {
      pageButton.addClass('current-page')
      pageButton.attr('aria-current', 'true')
    } else pageButton.click(()=> {
      pageOffset = (pageID - 1) * resultsShown
      // for now, because template.focus thing above isn't working, just move user back to search box
      // so they're at least at the top of the page, more or less
      $('#search_box').focus()
      setView('results')
    })
  ...
  try {
    let results = Promise.all(loadingList)
    ariaNotice("Search results updated.")
  } catch (e) {
    console.error(e)
    setView('notice', `Error loading results: ${e}`, 'data-error')
  }
}
