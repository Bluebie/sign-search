...
var videoRenderMode = 'video tag'


// check browser, and adjust video rendering to make stuff work on mobile safari
var uaParser = new UAParser(navigator.userAgent)
if (uaParser.getBrowser().name.match(/Safari/i)) videoRenderMode = 'img tag'
...
function ariaNotice(message, mode = 'polite') {
  $("#aria-notice").remove()
  let newNotice = $('<div id="aria-notice" tabindex="-1">').text(message).attr('aria-live', mode).css({"position":"absolute", "top": "-1000px"})
  $("#results").before(newNotice)
  return newNotice
}
...
async function renderResults() {
  ...
  for (let result of displayResults) {
    ...
    let video
    if (videoRenderMode == 'video tag') {
      video = $('<video class="video_thumb" muted preload autoplay loop playsinline></video>')
    } else if (videoRenderMode == 'img tag') {
      video = $('<img class="video_thumb">')
    }
    ...
    loadingList.push(new Promise((resolve, reject)=> {
      ...
      .then(({variations, outOfDate, data: def})=> {
        ...
        let setVariant = (adjust)=> {
          ...
          template.find(".video_player .video_thumb").remove()
  ...
  try {
    ...
    ariaNotice("Search results updated.", "off").focus()
  } catch (e) {
    console.error(e)
