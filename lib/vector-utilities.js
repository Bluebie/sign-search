...
const VU = {
  ...
  mean: (...list)=> VU.multiply([VU.add(...list)], VU.build(1 / list.length, list[0].length))[0],
  ...
  diversity: (...vectors)=> {
    ...
    let meanVector = VU.mean(...vectors)
    // calculate how far from the center point each vector is
    return vector.map(term => VU.distanceSquared(term, meanVector)) 
  }
}
