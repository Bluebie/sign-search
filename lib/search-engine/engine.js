  ...
  async query(queryText) {
    ...
    let results = await this.searchLibrary.query((result)=> {
      ...
      let distances = result.terms.map(resultTerm => {
        return Math.min(...query.keywords.map(queryTerm => {
          if (queryTerm.constructor == SearchQuery.WordVector) {
            if (typeof(resultTerm) !== 'string') {
              return VU.distanceSquared(queryTerm.vector, resultTerm)
            }
          } else { // plaintext keyword object
            if (typeof(resultTerm) == 'string') {
              if (queryTerm.text == resultTerm) {
                return 0
              } else if (queryTerm.text.toLowerCase() == resultTerm.toLowerCase()) {
                return 0.25
              }
            }
          }
          return 10 // bad/incompatible match between this keyword pair
        }))
      })
