...
class SearchEngine {
  ...
  // accepts a result returned by @query, and asyncronously fetches a definition we can use for presentation to user
  async fetch(result) {
    await this.awaitReady()

    let def = await this.searchLibrary.fetchDef(result)
    return {
      id: `${def.data.spider}:${def.data.key || def.data.link}`,
      provider: def.data.spider,
      title: def.data.title || (def.data.keywords || def.data.glossList).join(', '),
      distance: def.result.distance,
      media: def.media,
      link: def.data.link,
      tags: def.result.entity.tags,
      body: def.data.body,
    }
  }
