...
class SearchResultTile {
  ...
  constructor (config = {}) {
    ...
    this.forcedWarnings = []
    this.classList = new Set(['result', ...(config.classList || [])])
  }
  ...
  toPopulatedHTML () {
    const classes = new Set([...this.classList, ...this.getWarnings().map(x => x.type)])
    const component = html`
    <div class="${[...classes].join(' ')}" id="${this.toID()}">
      ${this.toMediaViewer()}
  ...
  toPlaceholderHTML () {
    const classes = new Set([...this.classList, ...this.getWarnings().map(x => x.type), 'placeholder'])
