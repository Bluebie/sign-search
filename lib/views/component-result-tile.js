...
class SearchResultTile {
  ...
  toPopulatedHTML() {
    ...
    <div class="result ${this.getWarnings().map(x => x.type).join(' ')}" id="${this.toID()}">
      ${this.toMediaViewer()}
      <h2 class="keywords">
        <a href="${this.result.link}" referrerpolicy="origin" rel="external">${this.result.title || this.result.keywords.join(', ')}</a>
      </h2>
      <cite class="link">
      ...
      ${this.getWarnings().map(warning => html`<div class="alert ${warning.type}">${icon('alert')} ${warning.text}</div>`)}
