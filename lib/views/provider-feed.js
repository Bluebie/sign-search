...
class FeedProvider extends base {
  async load() {
    ...
    let newsfeedGot = await got(signSearchConfig.newsRSS)
    let newsfeed = await xml2js.parseStringPromise(newsfeedGot.body)
  ...
  toHTML() {
    ...
    this.visibleUpdates.reverse().forEach(entry => {
      ...
      let providerLink = html`<a href="${providerURL}" class="provider-link p-author h-card">${providerName}</a>`
      let verb = entry.verb || 'documented'
      let entryLink = html`<a href="${entry.link}" class="entry-link p-name u-url">${[...entry.words].flat(2).slice(0,3).join(', ').trim()}</a>`
