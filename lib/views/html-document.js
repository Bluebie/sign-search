...
const fsutil = appRootPath.require('lib/sign-search-utils')
...
class DocumentTemplate {
  ...
  toBodyHTML() {
    ...
    let pieces = []
    if (!this.disable.symbols) pieces.push(raw(fs.readFileSync(appRootPath.resolve('style/assets/icomoon/symbol-defs.svg'))))
    if (!this.disable.header) pieces.push(html`
    ...
    if (!this.disable.javascript) pieces.push(html`<script>
    ...
    if (!this.disable.javascript && !this.disable.analytics) pieces.push(html`<script>
    ...
    return html`<body class="${this.body.getPageType()}" ${Object.entries(this.body.getData()).map(([k,v])=> html` data-${k}="${v}"`)}></body>
      ${pieces}
    </body>`
  }
