...
class DocumentTemplate {
  ...
  toHeadHTML () {
    ...
    if ((today.getMonth() === 9 && today.getDate() === 31) || (today.getMonth() === 10 && today.getDate() === 1)) {
      pieces.push(`<link rel=stylesheet href="${decache('style/halloween.css')}">`)
    }
    // new years eve theme
    if ((today.getMonth() === 11 && today.getDate() === 31) || (today.getMonth() === 0 && today.getDate() === 1)) {
      pieces.push(`<link rel=stylesheet href="${decache('style/new-years.css')}">`)
    }

    if (!this.disable.javascript) pieces.push(html`<script id=bundleScript defer src="${decache('lib/bundle.js')}"></script>`)
