...
class DocumentTemplate {
  ...
  toHeadHTML() {
    return html`<head>
      ...
      <link rel=alternate title="Recently Added (RSS)" type="application/rss+xml" href="${fullURL("feeds/discovery.rss")}">
    ...
    return html`<body class="${this.body.getPageType()}" ${Object.entries(this.body.getData()).map(([k,v])=> html` data-${k}="${v}"`)}>
      ...
      <div id="pagination">
        ${this.body.pagination ? 
          this.body.pagination.urls.map((url, idx)=> {
            this.body.pagination.selected == url ?
              html`<a href="${url}" aria-label="Page ${idx+1}" aria-current="page">${idx+1}</a>`
            : html`<a href="${url}" aria-label="Page ${idx+1}">${idx+1}</a>`
          }) : ""
        }
      </div>
