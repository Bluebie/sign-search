  ...
  async finish() {
    ...
    this.writeQueue.add(async () => {
      ...
      for (let [key, entry] of Object.entries(content)) {
        ...
        updateTxt.push(`${this.name} ${entry.discoveryVerb || this.config.discoveryVerb} [${entry.title ? entry.title : entry.words.join(', ')}](${entry.link}) (timestamp: ${entry.timestamp || Date.now()})\n`)
      }
