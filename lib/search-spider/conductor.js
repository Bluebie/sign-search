...
class SpiderConductor {
  ...
  async finish() {
    ...
    this.writeQueue.add(async () => {
      ...
      // don't rewrite if there's no new information
      // disabled because
      ...
      // write out logs
      await Promise.all([
        fs.writeFile(`${this.nest.settings.datasetsPath}/update-log.cbor`, Buffer.concat(updateLog.map(x => cbor.encode(x)))),
        fs.writeFile(`${this.nest.settings.datasetsPath}/update-log.txt`, updateLog.map(entry => {
