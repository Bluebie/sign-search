...
class SpiderPluginBase {
  ...
  getTasks() {
    ...
    // find all the referenced subtasks and accumulate an array of every task key that decends from
    // "[]" starting with the root task "[]"" itself
    while (inputTasks.length > 0) {
      let taskKey = inputTasks.shift()
      outputTasks.push(taskKey)

      let data = this.taskIO[taskKey]
      if (data && data.subtasks && data.subtasks.length > 0) {
        data.subtasks.forEach(subtask => {
          let subtaskKey = JSON.stringify(subtask)
          if (!outputTasks.includes(subtaskKey)) {
            inputTasks.push(subtaskKey)
          }
        })
      }
    }

    return outputTasks.map(x => JSON.parse(x))
  }
