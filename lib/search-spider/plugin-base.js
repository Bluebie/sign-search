...
class SpiderPluginBase {
  ...
  async executeTask(args) {
    let taskKey = JSON.stringify(args)
    let taskInfo = this.taskIO[taskKey] = this.taskIO[taskKey] || { failed: false, subtasks: [] }
    taskInfo.timestamp = Date.now()
    try {
      let output = await this.index(...args)
      if (!output) throw new Error(`Task ${args.join(', ')} returned something other than an object`)
      let { subtasks, data } = output
  ...
  getTasks() {
    let taskList = []
    // gather function recurses through the tree of subtask references adding new tasks to the taskList
    // until taskList contains every task that's in the tree referenced somewhere downstream from the root []
    // task is in the list
    let gather = (taskKey) => {
      taskList.push(taskKey)

      if (this.taskIO[taskKey] && this.taskIO[taskKey].subtasks && this.taskIO[taskKey].subtasks.length > 0) {
        this.taskIO[taskKey].subtasks.forEach(x => {
          let subtask = JSON.stringify(x)
          if (!taskList.includes(subtask)) gather(subtask)
        })
      }
    }
    // start with the root task every spider uses
    gather("[]")
    // parse all the list entries and return
    return taskList.map(x => JSON.parse(x))
  }
  ...
  getNextTask() {
    ...
    for (let taskKey of tasks) {
      // attempt to lookup results of last time this task ran (if ever)
      let taskData = this.taskIO[JSON.stringify(taskKey)]
  ...
  getContent() {
    ...
    return Object.fromEntries(
      ...
      this.getTasks()
        ...
        .filter(x => x && !x.failed && x.data && x.data.length > 0)
  ...
  async openText(url) {
    let page = await got({ url, headers: {'User-Agent': signSearchConfig.userAgent}, timeout: parseMs(this.config.timeout || '30s') })
