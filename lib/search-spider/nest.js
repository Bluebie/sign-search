...
class SpiderNest {
  ...
  log (...text) {
    ...
    const date = new Date()
    this.writeQueue.add(() =>
      fs.appendFile(`${this.settings.logsPath}/build.txt`, `${date.toLocaleString()}: ${message}\n`)
    )
  }
  ...
  async runOneSpider (source, maxAgeString = false) {
    if (!this.spiders[source]) throw new Error('Unknown spider config')
  ...
  async buildDatasets (force = false) {
    ...
    for (const spiderName in this.spiders) {
      ...
      for (const entryID in spiderContent) {
        ...
        const definition = {
          ...
          keywords: entry.keywords || entry.words,
          tags: [...(spider.config.tags || []), ...(entry.tags || [])].filter((v, i, a) => a.indexOf(v) === i).map(x => `${x}`.toLowerCase()),
          link: entry.link,
          nav: entry.nav,
          body: entry.body,
          media: (entry.media || entry.videos).map(videoInfo => new OnDemandMediaLoader({ spider: spider.spider, sourceVideoCache, spiderName, videoInfo, log: (...a) => this.log(...a) })),
          provider: spiderName,
        ...
        const logEntry = {
          provider: (entry.author && entry.author.name) ? entry.author.name : spiderName,
          providerLink: (entry.author && entry.author.link) ? entry.author.link : spider.config.link,
          id: definition.id,
        ...
        if (existingEntry) {
          ...
          this.log(`New: ${definition.link} - ${definition.keywords}`)
    ...
    await Promise.all([
      ...
      fs.writeFile(`${this.settings.datasetsPath}/update-log.txt`,
