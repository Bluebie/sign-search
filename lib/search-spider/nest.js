...
class SpiderNest {
  ...
  async runInSeries(maxAgeString) {
    ...
    for (let source of Object.keys(this.configs)) {
      ...
      // spider it again if it's been longer than the configured spider interval, or no configured interval exists
      if (!this.spiders[source].config.interval || sourceAge > parseDuration(this.spiders[source].config.interval)) {
        ...
        this.log(`Skipping ${source} as it is not due to run for another ${prettyMs(sourceAge)}`)
      }
    }
  ...
  async buildDatasets() {
    ...
    let buildID = objectHash(buildIDs.sort(), { algorithm: 'sha256' }).slice(0, 8)
