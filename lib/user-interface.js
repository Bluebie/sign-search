...
class FindSignUI {
  ...
  renderHome(hash) {
    ...
    this.ariaNotice("Welcome to Find Sign. Enter a search query")
  ...
  async renderSearch(hash) {
    ...
    if (this.results.length == 0) {
      return this.visualNotice("No search results found", true)
    } else {
    ...
    let finishedLoading = Promise.all(this.displayResults.map(async (result, index) => {
      ...
      view.setData(await this.searchEngine.fetch(result))
    }))
  ...
  ariaNotice(text, ariaLive = 'polite') {
    ...
    let notice = html`<div id="aria_notice" tabindex="-1" aria-live="${ariaLive}" style="position: absolute; top: -1000px">${text}</div>`
