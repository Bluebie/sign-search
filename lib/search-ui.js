class SearchUI {
  ...
  async resultsViewRender(search, pageOffset = 0) {
    ...
    await Promise.all(Object.values(this.searchLibraries).map(async searchLibrary => {
      await Promise.all(querySets.searchTerms[/*searchLibrary.settings.vectorLibrary ||*/ this.settings.vectorLibraries[0]].map(async term => {
        // get the raw results based purely on term matching
        let rawResults = await searchLibrary.lookup([term], { sort: false, rankMode: 'min' })
        // filter for hashtags
    ...
    let searchResults = Object.values(resultsTable).sort((a,b)=> a.distance - b.distance)
