class SearchUI {
    ...
    await Promise.all(rawTerms.map(async (rawTerm) => {
      if (rawTerm.match(/^-#([a-zA-Z0-9-_]+)$/)) {
        ...
        await Promise.all(Object.keys(this.vectorLibraries).map(async (vectorLibraryName) => {
          ...
          // if the capitalised version didn't work, try again lowercased, might work!
          if (!vector) vector = await vectorLibrary.lookup(cleanedTerm.toLowerCase())
  ...
  async resultsViewRender(search, pageOffset = 0) {
    ...
    await Promise.all(Object.values(this.searchLibraries).map(async searchLibrary => {
      let results = await searchLibrary.lookup(querySets.searchTerms[/*searchLibrary.settings.vectorLibrary || */this.settings.vectorLibraries[0]], false)
      for (let result of results) {
        if (querySets.positiveTags.every(tag => result.entity.tags.includes(tag)) && querySets.negativeTags.every(antitag => !result.entity.tags.includes(antitag))) {
