...
class VectorLibraryWriter {
  ...
  async finish() {
    ...
    // build zipped version of index
    // everything else is range requests so zip wont work for that
    await writeFile(`${this.paths.index}.gz`, await util.promisify(zlib.gzip)(await readFile(this.paths.index), { level: 9 }))
