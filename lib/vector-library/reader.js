...
class VectorLibraryReader {
  ...
  // internal: decode a number to the specified `format`
  ...
  // internal: decode a vector in to the compressed format
  decodeVector(data) {
  ...
  async lookup(search) {
    ...
    let utf8Decoder = new TextDecoder()
    if (search.constructor != String) search = search.toString()
    ...
    while (buffer.byteLength > 0) {
      ...
      if (typeNum == 0) {
        ...
        if (!caseInsensitiveEmbeddings[word.toLowerCase()]) {
          caseInsensitiveEmbeddings[word.toLowerCase()] = data
        }
      } else {
    ...
    } else if (this.settings.caseInsensitive && caseInsensitiveEmbeddings[search.toLowerCase()]) {
      return this.decodeVector(caseInsensitiveEmbeddings[search.toLowerCase()])
    }
