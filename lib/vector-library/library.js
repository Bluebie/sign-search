...
class VectorLibrary {
  ...
  async getWordInfo (stringWord) {
    ...
    const wordHash = await this.config.digest(this.settings.hashFunction, fsutil.encodeUTF8(stringWord))
    const prefixID = parseInt(fsutil.bytesToPrefixBits(wordHash, this.settings.prefixBits), 2)
    const shardID = parseInt(fsutil.bytesToPrefixBits(wordHash, this.settings.shardBits), 2)
  ...
  async addDefinition (stringWord, vectorArray) {
    ...
    const { path: shardPath, filtered } = await this.getWordInfo(stringWord)

    // figure out what the largest number in the vector is to use as a scaling value
    const vectorScale = Math.max(...vectorArray.map(Math.abs))
    // scale the digits accordingly
    const scaledVector = vectorArray.map(x => x / vectorScale)
    // pack the numbers in to a buffer of integers
    const packedVector = fsutil.packFloats(scaledVector, this.settings.vectorBits)
    // cbor encode each piece and append them
    const appendData = Buffer.concat([filtered, vectorScale, packedVector].map(x => cbor.encode(x)))
  ...
  async lookup (stringWord) {
    ...
    const { path: shardPath, filtered } = await this.getWordInfo(stringWord)
    // load shard data
    const shardData = cbor.decodeAll(await this.config.fs.readFile(shardPath))
    // iterate through entries in this shard until we find a match
    for (const [chunkWord, vectorScale, packedVector] of fsutil.chunkIterable(shardData, 3)) {
      if (filtered === chunkWord) {
        const scaledVector = fsutil.unpackFloats(packedVector, this.settings.vectorBits, this.settings.vectorSize)
