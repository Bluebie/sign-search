...
class VectorLibraryReader {
  ...
  constructor() {
    ...
    this.ranges = {}
    this.mtime = null
    this.mtimeThreshold = 5 * 60 * 1000 // if files in vector library are more than 5 minutes different, reload
  ...
  async lookup(word) {
    ...
    if (Math.abs(stringsRaw.mtime - this.mtime) > this.mtimeThreshold) throw new Error(`Dataset Updated: vector library strings file out of sync with loaded index`)
    if (Math.abs(vectorsRaw.mtime - this.mtime) > this.mtimeThreshold) throw new Error(`Dataset Updated: vector library vectors file out of sync with loaded index`)
  ...
  async _fetchWeb(filename, start = null, end = null) {
    ...
    try {
      buffer.mtime = Date.parse(data.headers.get('last-modified'))
    } catch (err) {
      console.error(`Fetch Web API failed to access headers while loading ${filename}, faking mtime`)
      buffer.mtime = this.mtime || Date.now()
    }
    return buffer
  }
  ...
  _fetchNodeJS(filename, start = null, end = null) {
    ...
    this._fetch = async function(filename, start = null, end = null) {
      ...
      if (start === null || end === null) return (await fs.readFile(filename)).buffer
      // if it's a range request, we need to do something a bit more elaborate
      let fd = await fs.open(filename, 'r')
      const { buffer: data } = await fs.read(fd, new Uint8Array(end - start), 0, end - start, start)
      await fs.close(fd)
      data.mtime = (await fs.stat(filename)).mtimeMs
      return data.buffer
    }
