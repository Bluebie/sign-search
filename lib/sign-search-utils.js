...
let Util = {
  ...
  sintToBits: (number, size) => {
    ...
    let trueNumberSize = size - 1
    let encoded = Math.abs(number).toString(2)
  ...
  bitsToSint: (binaryString) => {
    ...
    let compliment = binaryString.slice(0, 1)
    let numericBits = binaryString.slice(1)
    let absolute = parseInt(numericBits, 2)
  ...
  packFloats: (floats, bitPrecision) => {
    let sints = floats.map(num => Math.round(num * ((2 ** (bitPrecision - 1) - 1))))
  ...
  unpackFloats: (bytes, bitPrecision, totalFloats) => {
    ...
    return Util.timesMap(totalFloats, index => {
      let sint = Util.bitsToSint(bits.slice(index * bitPrecision, (index + 1) * bitPrecision))
  ...
  times: (times, callback) => {
    let iter = Util.timesMapIterable(times, callback)
  ...
  chunkIterable: function*(sliceable, chunkSize, maxChunks = Infinity) {
    let input = sliceable[Symbol.iterator]()
    while (maxChunks > 0) {
      let chunk = []
  ...
  fetchLikeFile: async (path) => {
    let data = await fetch(path, { mode: 'same-origin', credentials: 'omit' })
